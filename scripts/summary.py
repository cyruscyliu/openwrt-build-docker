...
def update_support_list(build_dir):
    pool = Pool(20)

    for owver_ibhash in os.listdir(build_dir):
        full_path = os.path.join(build_dir, owver_ibhash)
        ...
        print('[+] found {} {}'.format(hash_of_image_builder, openwrtver))
        pool.apply_async(
    ...
    print('[+] save as summary.yaml')
    yaml.safe_dump(support_list, open('summary.yaml', 'w'))


if __name__ == '__main__':
    if len(sys.argv) != 2:
        print('usage ./{} build_dir'.format(sys.argv[0]))
        exit(-1)
    update_support_list(sys.argv[1])
